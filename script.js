...
// TASK SYSTEM WITH COMPLETED TASK HIDING & ALL DONE MESSAGE
...
function loadTasks(completedTaskIds = {}) {
  ...
  db.ref('tasks').on('value', snap => {
    ...
    // FILTER OUT COMPLETED TASKS (HIDE COMPLETED TASKS)
    const remainingTasks = userVipTasks.filter(t => !completedTaskIds[t.id]);

    // CHECK IF ALL TASKS COMPLETED
    if (userVipTasks.length > 0 && remainingTasks.length === 0) {
      container.innerHTML = `
        <div class="content-card" style="text-align:center; padding:35px 20px;">
          <i class="fa-solid fa-circle-check" style="font-size:50px; color:#10b981; margin-bottom:12px;"></i>
      ...
      if (limitReached) {
        container.innerHTML = `
          <div class="content-card" style="text-align:center; padding:35px 20px;">
            <i class="fa-solid fa-circle-check" style="font-size:50px; color:#10b981; margin-bottom:12px;"></i>
            <h3 style="font-size:18px; color:var(--text-main); font-weight:800;">আপনি ইতিমধ্যেই আজকের সকল টাস্ক সম্পন্ন করেছেন!</h3>
            <p style="font-size:12px; color:var(--text-muted); margin-top:6px;">আজকের সর্বোচ্চ টাস্কের সীমানায় পৌঁছে গেছেন।</p>
          </div>
        `;
        return;
      }
...
window.saveEWalletSetup = async function(e) {
  ...
  // IF PREVIOUS WALLET WAS DIFFERENT, REMOVE OLD BIND
